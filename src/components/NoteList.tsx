...
export const NoteList: React.FC<NoteListProps> = ({ selectedNoteId, onSelectNote, onNewNote }) => {
  ...
  return (
    <div className="flex flex-col h-full w-80 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800">
      <div className="p-4 flex flex-col gap-4">
        <div className="flex items-center justify-between">
          ...
          <button
            onClick={onNewNote}
            className="p-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-500 transition-colors shadow-sm"
          >
            <Plus size={20} />
          </button>
        </div>
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full bg-slate-100 dark:bg-slate-800 border-none rounded-xl py-2 pl-10 pr-4 text-sm focus:ring-2 ring-indigo-500 outline-none"
          />
        </div>
      </div>
